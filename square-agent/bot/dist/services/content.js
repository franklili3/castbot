...
async function loadCopyTradingData() {
    try {
        ...
        const filePath = join(homedir(), 'clawd/data/binance-content/copy-trading/latest.json');
...
async function getFundingRate() {
    ...
    const fallback = { btcRate: 0, ethRate: 0 };
    try {
        const [btcRes, ethRes] = await Promise.all([
...
const SHORT_TEMPLATES = [
    ...
    // ────────────── P2 模板 ──────────────
    // P2-1：资金费率热力图
    { name: '📊 资金费率热力图', tag: 'value', fn: (ctx) => {
            const f = ctx.funding;
            const m = ctx.market;
            // 资金费率解读
            const interpret = (rate) => {
                if (rate > 30)
                    return '🔴 极度看多拥挤';
                if (rate > 15)
                    return '🟠 偏多杠杆';
...
// 读取当日热点报告作为写作参考
async function loadHotTopicsReport() {
    try {
        const today = new Date().toISOString().slice(0, 10);
        const { readFileSync } = await import('fs');
        const { join } = await import('path');
        const { homedir } = await import('os');
        const reportPath = join(homedir(), 'clawd/data/binance-content/analytics', `daily-hot-topics-${today}.md`);
...
export async function generateContent(style, contentType, context, enabledTemplateNames, serverApiConfig) {
    ...
    const tplCtx = { market, copyTrading, funding, hotReport };
    // 构建可用模板池
    let localTemplates;
    let defaultTopics;
    switch (contentType) {
    ...
    // 如果过滤后为空，回退到全部可用模板（避免没有模板可选）
    if (availableNames.length === 0)
        availableNames = allNames.filter(n => enabledTemplateNames?.includes(n) || !enabledTemplateNames || enabledTemplateNames.length === 0);
    ...
    for (const n of valueNames)
        weightedPool.push({ name: n, weight: 80 / totalValue });
    // 推广池总权重固定 20，平均分配给可用推广模板
    for (const n of promoNames)
        weightedPool.push({ name: n, weight: 20 / totalPromo });
    // 如果推广池为空，全部权重给价值池
    if (promoNames.length === 0) {
        for (const wp of weightedPool)
            wp.weight = 100 / valueNames.length;
    }
    ...
    if (serverNames.includes(chosenName) && serverApiConfig) {
        ...
        try {
            ...
            console.error('Server template generation failed:', e.message);
...
export const SERVER_SHORT_TEMPLATE_IDS = {
