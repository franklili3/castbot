  ...
  tell front window
    tell active tab
      ...
      execute front window's active tab javascript "document.querySelector('[aria-expanded]').setAttribute('aria-expanded','true'); void(0);"
        ...
        await sleep(1000);
        // 逐行输入内容（用 peekaboo 如果可用，否则用 AppleScript）
        let usePeekaboo = false;
        try {
            execSync('which peekaboo', { stdio: 'pipe' });
            usePeekaboo = true;
        }
        catch { }
        if (usePeekaboo) {
            ...
            for (const line of lines) {
                if (line.trim() === '') {
                    execSync('peekaboo press return --app "Google Chrome"');
                }
                else {
                    execSync(`peekaboo type ${JSON.stringify(line)} --app "Google Chrome"`);
                    execSync('peekaboo press return --app "Google Chrome"');
                }
                await sleep(200);
            }
...
// ============ 回传状态 ============
async function reportStatus(postId, status, binancePostId, error) {
