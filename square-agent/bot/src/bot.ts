...
import { generateContent, SHORT_TEMPLATE_NAMES, POLL_TEMPLATE_NAMES, ALL_SHORT_TEMPLATE_NAMES, SERVER_SHORT_TEMPLATE_NAMES, PROMO_TEMPLATE_NAMES } from './services/content.js';
import { publishToBinanceSquare } from './services/publisher.js';
import _path from 'path';
import os from 'os';

// 运行时数据路径（与 src/paths.mjs 保持一致；schema.ts 不能 import .mjs，inline 同样语义）
const _dataRoot = process.env.SQUARE_AGENT_DATA_DIR || _path.join(os.homedir(), '.square-agent');
const _tgPendingDir = process.env.SQUARE_AGENT_TG_PENDING_DIR || _path.join(_dataRoot, 'tg-pending');
const _newsDataDir = process.env.SQUARE_AGENT_NEWS_DATA_DIR || _path.join(_dataRoot, 'news-pipeline');
...
// ============ 话题标签推导（从评分结果映射新闻主题）============
const _CATEGORY_TOPIC_MAP: [RegExp, string][] = [
  [/etf/i, 'ETF'],
  [/安全|黑客/i, '安全事件'],
  [/sec|监管|执法/i, 'SEC'],
  [/fomc|议息|会议/i, '美联储会议'],
...
function deriveTopicsFromItem(item: any): string {
  if (!item) return 'BTC';
  ...
  if (cat) {
    let matched = false;
    for (const [re, tag] of _CATEGORY_TOPIC_MAP) {
...
// ============ API helpers ============
async function tgApi(method: string, body?: Record<string, any>): Promise<any> {
...
async function editMessage(chatId: number, messageId: number, text: string, extra?: Record<string, any>) {
...
async function editMessageWithKeyboard(chatId: number, messageId: number, text: string, keyboard: any) {
...
async function handleCallback(callbackQuery: any) {
  ...
  const typeLabels: Record<string, string> = { short: '📝 短贴', poll: '📊 投票贴', promo: '📢 推广贴' };
  ...
  if (styleMatch) {
    ...
    const names: Record<string, string> = { aggressive: '🔥 激进型', balanced: '⚖️ 稳健型', quantitative: '📊 量化型' };
    await answerCallback(callbackQuery.id, '✅ 已选择');
    // Generate agent token
    const token = db.prepare('SELECT agent_token FROM users WHERE telegram_id = ?').get(user.telegram_id) as any;
    await editMessage(chatId, msgId,
      `✅ 内容风格：${names[style] || style}\n\n` +
      `🎉 账号已就绪！\n\n` +
      `开始使用：\n` +
      `• /preview - 生成第一篇内容\n` +
      `• /settings - 调整偏好设置\n\n` +
  ...
  if (approveMatch) {
    ...
    if (!post) { await answerCallback(callbackQuery.id, '❌ 不存在'); return; }
  ...
  if (regenMatch) {
    ...
    await sendMessageWithKeyboard(chatId,
      `📝 内容预览 (#${newPost.id}) [${typeLabels[contentType] || contentType} | ${newContent.templateName}]\n\n${newContent.content}\n\n---\n选择操作：`,
      {
        inline_keyboard: [
  ...
  if (data.startsWith('cp_pub:')) {
    ...
    if (post.status === 'approved' || post.status === 'published') {
      await answerCallback(callbackQuery.id, '⚠️ 已处理');
      await editMessage(chatId, msgId, `⚠️ 帖子 #${postId} 已在队列中（状态: ${post.status}），不要重复操作。`);
      return;
    }
  ...
  if (data.startsWith('cp_del:')) {
    const postId = parseInt(data.split(':')[1]);
    try { postService.rejectPost(postId); } catch {}
    await answerCallback(callbackQuery.id, '🗑️ 已丢弃');
  ...
  if (data.startsWith('tn_pub:')) {
    ...
    const pendingPath = _path.join(_tgPendingDir, `${newsId}.json`);
    try {
      ...
      // 防重复：检查相同内容是否已存在
      const contentHash2 = pending.message.substring(0, 100);
      const existing2 = (await import('./db/schema.js')).default.prepare(
        "SELECT id, status FROM posts WHERE user_id = ? AND substr(content,1,100) = ? AND status IN ('approved','published') ORDER BY id DESC LIMIT 1"
      ).get(user.id, contentHash2);
      if (existing2) {
        await answerCallback(callbackQuery.id, '⚠️ 已在队列');
        await editMessage(chatId, msgId, `⚠️ 这条内容已加入过队列（帖子#${existing2.id}），不要重复发布。`);
        return;
      }
      // 用 better-sqlite3 写入
      const insertStmt = (await import('./db/schema.js')).default.prepare(
        "INSERT INTO posts (user_id, content, topics, status, scheduled_at) VALUES (?, ?, 'BTC', 'approved', datetime('now'))"
      );
      const insertResult = insertStmt.run(user.id, pending.message);
      const newId = String(insertResult.lastInsertRowid);
  ...
  if (data.startsWith('tn_del:')) {
    const newsId = data.split(':').slice(1).join(':');
    const fs = await import('fs');
    try { fs.unlinkSync(_path.join(_tgPendingDir, `${newsId}.json`)); } catch {}
    await answerCallback(callbackQuery.id, '🗑️ 已丢弃');
  ...
  if (data.startsWith('pub:')) {
    ...
    try {
      ...
      const interpretedPath = _path.join(_newsDataDir, 'interpreted-news.json');
      const statePath = _path.join(_newsDataDir, 'pipeline-state.json');
      ...
      if (fs2.existsSync(interpretedPath)) {
        ...
        newsItem = interpreted.find((n: any) => n.id === newsId);
        if (newsItem?.interpretedContent) content = newsItem.interpretedContent;
      }
      ...
      if (!content && fs2.existsSync(statePath)) {
        ...
        newsItem = (state.outputs || []).find((o: any) => o.id === newsId);
        if (newsItem?.interpretedContent) content = newsItem.interpretedContent;
      }
      ...
      const existing = (await import('./db/schema.js')).default.prepare(
        "SELECT id, status FROM posts WHERE user_id = ? AND substr(content,1,100) = ? AND status IN ('approved','published') ORDER BY id DESC LIMIT 1"
      ).get(user.id, contentHash);
      if (existing) {
        await answerCallback(callbackQuery.id, '⚠️ 已在队列');
        await editMessage(chatId, msgId, `⚠️ 这条新闻已加入过队列（帖子#${existing.id}），不要重复发布。`);
        return;
      }
      // 用 better-sqlite3 写入
      const _topicStr = deriveTopicsFromItem(newsItem);
      const insertStmt2 = (await import('./db/schema.js')).default.prepare(
        "INSERT INTO posts (user_id, content, topics, status, scheduled_at) VALUES (?, ?, ?, 'approved', datetime('now'))"
      );
      const insertResult2 = insertStmt2.run(user.id, content, _topicStr);
      const newId = String(insertResult2.lastInsertRowid);
      await answerCallback(callbackQuery.id, '✅ 已加入队列');
      await editMessage(chatId, msgId, `✅ 已加入发布队列！\n帖子ID: #${newId}\n\nPublisher Agent 会自动发布到币安广场。`);
    } catch (e: any) {
      await answerCallback(callbackQuery.id, '❌ 错误');
  ...
  // Settings
  if (data === 'settings_review_mode') {
    const newMode = user.review_mode === 'auto' ? 'manual' : 'auto';
    userService.updateReviewMode(user.telegram_id, newMode);
    const modeLabel = newMode === 'auto' ? '🤖 自动发布' : '✋ 人工审核';
    await answerCallback(callbackQuery.id, `✅ 已切换为${modeLabel}`);
    // 刷新 settings 消息
    const styleNames: Record<string, string> = { aggressive: '🔥 激进型', balanced: '⚖️ 稳健型', quantitative: '📊 量化型' };
    const cmdTypeLabels: Record<string, string> = { short: '📝 短贴', poll: '📊 投票贴' };
    const contentTypeDisplay = (user.content_types || '').split(',').filter(Boolean).map((t: string) => cmdTypeLabels[t] || t).join('、') || '未设置';
    ...
    await editMessageWithKeyboard(chatId, msgId,
      `⚙️ 当前设置\n\n🎨 风格：${styleNames[user.style] || user.style}\n📊 频率：${user.frequency} 篇/天\n📋 内容类型：${contentTypeDisplay}\n📈 行情短贴模板：${localDisplay}\n📰 新闻短贴模板：${newsDisplay}\n📊 投票贴模板：${pollDisplay}\n📢 推广贴模板：${promoDisplay}\n🔍 审核模式：${modeLabel}\n🌐 语言：${user.language === 'zh' ? '中文' : user.language === 'en' ? 'English' : user.language}\n🪙 币种：${user.coins || 'BTC,ETH'}\n\n修改设置：`,
      {
        inline_keyboard: [
          ...
          [{ text: `🔍 审核模式：${modeLabel}`, callback_data: 'settings_review_mode' }],
          [{ text: `🌐 语言：${user.language === 'zh' ? '中文' : user.language === 'en' ? 'English' : user.language}`, callback_data: 'settings_language' }, { text: `🪙 币种：${user.coins || 'BTC,ETH'}`, callback_data: 'settings_coins' }],
        ]
      }
    );
    return;
  ...
  if (data === 'settings_language') {
    ...
    const langRows = Object.entries(langNames).map(([code, name]) => [
      { text: `${user.language === code ? '✅ ' : ''}${name}`, callback_data: `set_lang_${code}` }
    ]);
    await sendMessageWithKeyboard(chatId, '🌐 选择发帖语言：', { inline_keyboard: langRows });
    return;
  }

  if (data.startsWith('set_lang_')) {
    const lang = data.replace('set_lang_', '');
    userService.updateLanguage(user.telegram_id, lang);
    const langNames: Record<string, string> = { zh: '中文', en: 'English', ja: '日本語', ko: '한국어' };
    await answerCallback(callbackQuery.id, `✅ 语言已切换为${langNames[lang] || lang}`);
    await sendMessage(chatId, `✅ 发帖语言已更新为 ${langNames[lang] || lang}。\n新闻管道将在下一轮生效。`);
    return;
  }

  if (data === 'settings_coins') {
    await answerCallback(callbackQuery.id);
    const currentCoins = (user.coins || 'BTC,ETH').split(',');
    const allCoins = ['BTC', 'ETH', 'SOL', 'BNB', 'XRP', 'DOGE', 'ADA', 'AVAX', 'DOT', 'LINK', 'TON'];
    const coinButtons = allCoins.map(c => [
      { text: `${currentCoins.includes(c) ? '✅ ' : ''}${c}`, callback_data: `toggle_coin_${c}` }
    ]);
    coinButtons.push([{ text: '✅ 完成', callback_data: 'settings_coins_done' }]);
    await sendMessageWithKeyboard(chatId, `🪙 选择关注的币种（当前：${user.coins || 'BTC,ETH'}）：`, { inline_keyboard: coinButtons });
    return;
  }

  if (data.startsWith('toggle_coin_')) {
    const coin = data.replace('toggle_coin_', '');
    const current = (user.coins || 'BTC,ETH').split(',').filter(Boolean);
    let newList: string[];
    if (current.includes(coin)) {
      newList = current.filter(c => c !== coin);
      if (newList.length === 0) newList = ['BTC']; // 至少保留一个
    } else {
      newList = [...current, coin];
    }
    const coinsStr = newList.join(',');
    userService.updateCoins(user.telegram_id, coinsStr);
    await answerCallback(callbackQuery.id, `${current.includes(coin) ? '移除' : '添加'} ${coin}`);
    // 刷新按钮
    const allCoins = ['BTC', 'ETH', 'SOL', 'BNB', 'XRP', 'DOGE', 'ADA', 'AVAX', 'DOT', 'LINK', 'TON'];
    const coinButtons = allCoins.map(c => [
      { text: `${newList.includes(c) ? '✅ ' : ''}${c}`, callback_data: `toggle_coin_${c}` }
    ]);
    coinButtons.push([{ text: '✅ 完成', callback_data: 'settings_coins_done' }]);
    await editMessageWithKeyboard(chatId, msgId, `🪙 选择关注的币种（当前：${coinsStr}）：`, { inline_keyboard: coinButtons });
    return;
  }

  if (data === 'settings_coins_done') {
    await answerCallback(callbackQuery.id, '✅ 币种设置已保存');
    await sendMessage(chatId, `✅ 关注币种：${user.coins || 'BTC,ETH'}\n新闻管道将在下一轮生效。`);
    return;
  }
  ...
  // ============ 内容子模板设置 ============
  const makeSubTplButtons = (group: string, names: string[], currentNames: string[]) => {
    const btns: any[][] = [];
  ...
  if (toggleSubTplMatch) {
    ...
    const nameMap: Record<string, string[]> = { local: SHORT_TEMPLATE_NAMES, news: SERVER_SHORT_TEMPLATE_NAMES, poll: POLL_TEMPLATE_NAMES, promo: PROMO_TEMPLATE_NAMES };
    const allNames = nameMap[group];
    const toggledName = allNames[idx];
    if (!toggledName) { await answerCallback(callbackQuery.id, '❌ 错误'); return; }
    ...
    const groupLabels: Record<string, string> = { local: '📈 行情短贴模板', news: '📰 新闻短贴模板', poll: '📊 投票贴模板', promo: '📢 推广贴模板' };
  ...
  if (saveSubTplMatch) {
    ...
    const groupLabels2: Record<string, string> = { local: '📈 行情短贴模板', news: '📰 新闻短贴模板', poll: '📊 投票贴模板', promo: '📢 推广贴模板' };
    await answerCallback(callbackQuery.id, '✅ 已保存');
    ...
    try {
      ...
      if (!newsItem) {
        try {
          ...
          newsItem = interp.find((o: any) => o.id === newsId);
        } catch {}
      }
      ...
      const _topics = deriveTopicsFromItem(newsItem).split(',');
      const newPost = postService.createPost(user.id, newsItem.interpretedContent, _topics, '📰 新闻快讯');
...
async function handleCommand(chatId: number, from: any, command: string, parts?: string[]) {
  const tgId = from.id;
  const user = userService.findByTelegramId(tgId);
  ...
  switch (command) {
    ...
    case 'token':
      if (!user) { await sendMessage(chatId, '❌ 请先 /start 注册'); return; }
      const tokenRow = db.prepare('SELECT agent_token FROM users WHERE telegram_id = ?').get(tgId) as any;
      await sendMessage(chatId,
    ...
    case 'preview': {
      ...
      if (user.review_mode === 'auto') {
        postService.approvePost(post.id);
        await sendMessage(chatId,
          `🤖 自动发布已入队 (#${post.id}) [${content.templateName}]，等待 Publisher 发布后通知结果。`
        );
      } else {
        await sendMessageWithKeyboard(chatId,
          `📝 内容预览 (#${post.id}) [${cmdTypeLabels[contentType] || contentType} | ${content.templateName}]\n\n${content.content}\n\n---\n选择操作：`,
          {
            inline_keyboard: [
    ...
    case 'settings': {
      ...
      const pollDisplay = enabledPoll.length > 0 ? enabledPoll.join('、') : '未设置';
      const promoDisplay = enabledPromo.length > 0 ? enabledPromo.join('、') : '未设置';
      const reviewModeDisplay = user.review_mode === 'auto' ? '🤖 自动发布' : '✋ 人工审核';
      await sendMessageWithKeyboard(chatId,
        `⚙️ 当前设置\n\n🎨 风格：${styleNames[user.style] || user.style}\n📊 频率：${user.frequency} 篇/天\n📋 内容类型：${contentTypeDisplay}\n📈 行情短贴模板：${localDisplay}\n📰 新闻短贴模板：${newsDisplay}\n📊 投票贴模板：${pollDisplay}\n📢 推广贴模板：${promoDisplay}\n🔍 审核模式：${reviewModeDisplay}\n🌐 语言：${user.language === 'zh' ? '中文' : user.language === 'en' ? 'English' : user.language}\n🪙 币种：${user.coins || 'BTC,ETH'}\n\n修改设置：`,
        {
          inline_keyboard: [
            [{ text: '🎨 修改风格', callback_data: 'settings_style' }, { text: '📊 修改频率', callback_data: 'settings_frequency' }],
            [{ text: '📋 修改内容类型', callback_data: 'settings_content_types' }],
            [{ text: '📈 修改行情短贴模板', callback_data: 'settings_local_tpl' }, { text: '📰 修改新闻短贴模板', callback_data: 'settings_news_tpl' }],
            [{ text: '📊 修改投票贴模板', callback_data: 'settings_poll_tpl' }, { text: '📢 修改推广贴模板', callback_data: 'settings_promo_tpl' }],
            [{ text: `🔍 审核模式：${reviewModeDisplay}`, callback_data: 'settings_review_mode' }],
            [{ text: `🌐 语言：${user.language === 'zh' ? '中文' : user.language === 'en' ? 'English' : user.language}`, callback_data: 'settings_language' }, { text: `🪙 币种：${user.coins || 'BTC,ETH'}`, callback_data: 'settings_coins' }],
          ]
        }
      );
    ...
    case 'history':
      ...
      if (allPosts.length === 0) { await sendMessage(chatId, '📭 还没有内容记录。'); return; }
      const emoji2: Record<string, string> = { draft: '📝', approved: '⏳', published: '✅', rejected: '🗑️', failed: '❌' };
    ...
    case 'unregister':
      if (!user) { await sendMessage(chatId, '❌ 你还没有注册。'); return; }
    ...
    case 'schedule':
      ...
      if (subCmd === 'list') {
        ...
        if (!schedId) { await sendMessage(chatId, '用法: /schedule remove 任务ID'); return; }
        try {
    ...
    case 'admin_pending':
      ...
      if (pending.length === 0) { await sendMessage(chatId, '✅ 没有待审核用户'); return; }
            ...
            if (msg.text?.startsWith('/')) {
              ...
              if ((cmd === 'admin_approve' || cmd === 'adminapprove') && parts[1]) {
                ...
                if (!isNaN(tid)) {
                  ...
                  const row = db.prepare('SELECT * FROM users WHERE telegram_username = ?').get(arg) as any;
                  if (row) targetUser = row;
                }
                if (!targetUser) {
...
const _notifyDir = (() => {
  ...
  try { readdirSync(c1); return c1; } catch {}
  try { readdirSync(c2); return c2; } catch {}
...
// 防重复通知：记录已通知的 postId
const _notifiedPosts = new Set();

setInterval(async () => {
  try {
    const files = readdirSync(_notifyDir).filter((f: string) => f.endsWith('.json'));
    ...
    for (const file of files) {
      ...
      try {
        ...
        // 防重复：同一 postId 只通知一次
        const notifyKey = `${notify.postId}_${notify.status}`;
        if (_notifiedPosts.has(notifyKey)) continue;
        _notifiedPosts.add(notifyKey);
        
        const post = (db as any).prepare('SELECT id, user_id, status, substr(content,1,80) as preview FROM posts WHERE id = ?').get(notify.postId);
        if (!post) continue;
        ...
        // 清理 Set（保留最近100条防止内存泄漏）
        if (_notifiedPosts.size > 100) {
          const arr = Array.from(_notifiedPosts);
          arr.slice(0, 50).forEach(k => _notifiedPosts.delete(k));
        }
      } catch (err) {
